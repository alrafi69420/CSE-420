...
bool function_declaration(string name){
	symbol_info* temp_symbol = new symbol_info(name, "ID");
	symbol_info* result = table->lookup(temp_symbol);
	delete temp_symbol;
	return result != NULL && result->get_is_function();
}

bool var_in_curr_scope(string name){
    symbol_info* temp_symbol = new symbol_info(name, "ID");
    symbol_info* result = table->lookup_current_scope(temp_symbol);
    delete temp_symbol;
    return result != NULL;
}
...
declaration_list : declaration_list COMMA ID
		  {
			...
			if(var_in_curr_scope($3->getname())) {
 		  ...
 		  |ID
 		  {
            ...
            // you may need to store the variable names to insert them in symbol table here or later
			if(var_in_curr_scope($1->getname())) {
